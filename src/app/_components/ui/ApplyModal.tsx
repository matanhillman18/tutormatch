  ...
  const handleSubmit = async () => {
    ...
    try {
      const { error: dbErr } = await supabase.from('applications').insert({
        tutor_id: session.id, request_id: request.id,
        message: message.trim(), availability: availability.trim(),
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        ...
        {success ? (
          <div className="px-6 py-12 flex flex-col items-center text-center">
            <div className="w-14 h-14 bg-green-50 rounded-full flex items-center justify-center mb-4"><CheckCircle size={28} className="text-green-500" /></div>
          ...
          <div className="px-6 py-5 space-y-4">
            <div className="bg-[#fafaf9] border border-[#e8e4db] rounded-xl p-4">
              <p className="text-[10px] font-semibold text-[#9c9a93] uppercase tracking-wider mb-2">{T(t, 'modal.apply.summary')}</p>
              <p className="text-sm text-[#3d3d3a] leading-relaxed line-clamp-2">{request.description}</p>
              <div className="flex items-center gap-3 mt-3 text-xs text-[#6f6d66]">
                <span className="font-medium text-[#a06e12]">{T(t, 'modal.apply.budget')}: ₪{request.budget}/hr</span>
                <span>·</span><span>{lessonLabel}</span>
                {request.location && <><span>·</span><span>{request.location}</span></>}
              </div>
            </div>

            <div>
              <label className="block text-sm font-medium text-[#3d3d3a] mb-1.5">{T(t, 'modal.apply.msg')}</label>
              <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder={T(t, 'modal.apply.msg.ph')} rows={3} className={`${inputCls} resize-none`} />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#3d3d3a] mb-1.5">{T(t, 'modal.apply.avail')}</label>
              <input type="text" value={availability} onChange={e => setAvailability(e.target.value)} placeholder={T(t, 'modal.apply.avail.ph')} className={inputCls} />
            </div>

            <div className="flex items-start gap-2 px-3 py-2.5 bg-[#faefd9] border border-[#f3d99b] rounded-xl text-xs text-[#a06e12]">
              {T(t, 'modal.apply.notify')}
            </div>

            {error && <div className="px-4 py-2.5 bg-red-50 border border-red-100 rounded-xl text-red-600 text-xs">{error}</div>}

            <button onClick={handleSubmit} disabled={loading}
              className="w-full py-3.5 bg-[#111110] text-white font-medium rounded-xl hover:bg-[#e5a82e] hover:text-[#111110] transition-all duration-200 text-sm flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed">
              {loading ? <><span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" />{T(t, 'modal.apply.sending')}</> : <><Send size={14} />{T(t, 'modal.apply.submit')}</>}
            </button>
          </div>
        )}
      </div>
